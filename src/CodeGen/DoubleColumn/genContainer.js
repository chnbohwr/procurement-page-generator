...
const ${upperCaseFirstChar(containerName)} = (props) => {
   ...
   */
  useEffect(() => {
    const selectedItem = _find(${lowerCaseFirstChar(containerName)}List, obj => obj.id === leftCsdb.selectedRowId);
    const newSubItems = selectedItem ? selectedItem.items : [];
    setSubItems(newSubItems);
  }, [JSON.stringify(${lowerCaseFirstChar(containerName)}List), leftCsdb.selectedRowId]);
    ...
    <InnerContainer>
      <div className="inner-content">
        <div className="content-header">
          <div className="title">${upperCaseFirstChar(containerName)}</div>
        </div>
        {/* 左邊 */}
